...
def send_fcm(tokens: List[str], title: str, body: str, data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Send push notifications via FCM HTTP v1.
    Data-only payload so web behaves consistently:
      - Foreground: onMessage in page
      - Background: onBackgroundMessage in SW
    """
    project_id = os.getenv("FIREBASE_PROJECT_ID")
    if not project_id:
        raise RuntimeError("FIREBASE_PROJECT_ID not set")
    ...
    # Ensure all values in data are strings (FCM v1 requirement)
    safe_data = {str(k): str(v) for k, v in (data or {}).items()}
    # put the "title/body" in data as well
    safe_data.setdefault("title", str(title))
    safe_data.setdefault("body", str(body))
    # Optional link SW can use when clicking
    safe_data.setdefault("link", "https://familiesfuel.com/orders")
