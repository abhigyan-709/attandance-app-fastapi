...
class ProductBase(BaseModel):
    ...
    description: Optional[str]
    price: float
    gst: float                       # percent, e.g. 5.0 for 5%
    unit: Optional[str]
    region: str                      # GI tag region
    sku: str
    category: Optional[str]
...
class OrderLine(BaseModel):
    ...
    quantity: int = Field(..., gt=0)
    subtotal: float                  # unit_price * quantity
    gst_percent: float               # product.gst at order time
    gst_amount: float                # subtotal * gst_percent / 100
...
class OrderFromCartCreate(BaseModel):
    """
    Payload the client sends to /cart/checkout.
    Cart items are taken from the user's server-side cart.
    """
    delivery_address: str
    contact_phone: str
    payment_method: str              # "cod" (for now)

class Order(BaseModel):
    id: str
    user: dict                       # { id, username, email }
    vendor: dict                     # serialized vendor snapshot
...
# Backward compatibility alias (same shape as OrderFromCartCreate)
class CartCheckoutRequest(OrderFromCartCreate):
    pass
